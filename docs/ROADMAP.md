...
- [ ] **動的な型発見 (Dynamic Type Discovery)**: Rust 側での型情報のハードコードを廃止し、Godot エディターへのイントロスペクションで解決する仕組みを導入。
- [ ] **スキーマ駆動開発の強化**: `schema.graphql` から Rust/GDScript のコードを自動生成/同期する仕組みの検討。
- [x] **WebSocket 移行**: HTTP/SSE から WebSocket に移行し、双方向・低遅延通信を実現。 ✅ 2024-12
- [ ] **トランザクション操作**: 複数のミューテーションを一括で適用し、Undo/Redo 単位を LLM の 1 ターンと同期。
- [x] **エラーハンドリングの構造化**: 詳細なエラーコードとスタックトレースを返却し、AI による自己修復能力を向上。 ✅ 2024-12
...
- [ ] `live_get_parse_errors` - スクリプト保存直後の構文エラー検知。
...
- [ ] `live_get_stack_frame_vars` - スタックフレームのローカル変数取得。
