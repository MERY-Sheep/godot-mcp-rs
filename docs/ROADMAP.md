...
LLM が呼び出せるのは以下の **3 つの GraphQL ツール** のみです。
すべての Godot 操作はこれらのツールを通じて実行されます。
...
> [!TIP]
> 個々の操作（`addNode`, `setProperty` 等）は GraphQL の **フィールド** として定義されており、
> スキーマをイントロスペクトすることで完全な API 仕様を取得できます。

### CLI ツール（開発者向け）

CI/CD パイプラインやシェルスクリプトから直接実行するための **55+ コマンド** を提供。
これらは MCP 経由では呼び出せません。

```bash
godot-mcp-rs tool --help  # 利用可能なコマンド一覧
```
...
| :------------------- | :--------------------------------------------- |
| 実装言語             | Rust                                           |
| リアルタイム操作     | ✅ 対応 (GraphQL ミューテーション / WebSocket) |
| Undo/Redo 統合       | ✅ 完全対応                                    |
| エディタープラグイン | ✅ GDScript                                    |
| CLI モード           | ✅ 対応                                        |
