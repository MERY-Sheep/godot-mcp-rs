...
このサーバーを MCP サーバーとして使用する場合（Claude Desktop / Cursor 経由）、利用可能なツールは以下の **3 本**のみです：

- `godot_query` - GraphQL クエリの実行
- `godot_mutate` - GraphQL ミューテーションの実行
- `godot_introspect` - GraphQL スキーマの取得

すべての操作（ノード追加、シーン作成等）は GraphQL ミューテーション経由で実行されます。利用可能な操作は `godot_introspect` で確認できます。

## GraphQL CLI コマンド (推奨)

本プロジェクトの主要な操作は GraphQL (GQL) インターフェースに統合されています。従来の個別ツール群も引き続き CLI から利用可能ですが、通常の MCP サーバーとしての利用や高度な自動化には GQL ツールの使用を推奨します。
...
AI がゲームの実行状態を把握し、ランタイムの課題を解決するための機能です。
